        ...
        <div class="col-12 col-md-6">
            ...
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb m-0 p-0">
                    ...
                    <li class="breadcrumb-item"><a href="{{ route('projections.index') }}" class="text-muted">Proyeksi Jabatan</a></li>
        ...
        <div class="col-12">
            <div class="d-flex align-items-center">
                ...
                @if ($projection['is_held_by_speedbump'])
                    <span class="badge bg-warning-subtle text-warning border border-warning-subtle fs-6 px-3 py-2">Tertahan Waktu</span>
                @elseif ($projection['is_ready_mathematically'])
                    <span class="badge bg-success-subtle text-success border border-success-subtle fs-6 px-3 py-2">Siap AK</span>
                @else
                    <span class="badge bg-primary-subtle text-primary border border-primary-subtle fs-6 px-3 py-2">Proses AK</span>
                @endif
            </div>
                ...
                <div class="card-body">
                    ...
                    <div class="row text-center mb-4">
                        ...
                        <div class="col-4">
                            ...
                            <span class="text-muted small">Defisit AK</span>
                        </div>
                    </div>
                    ...
                    <div class="mb-4">
                        <div class="d-flex justify-content-between mb-1">
                            ...
                            <span class="fw-medium">{{ number_format($projection['progress_percentage'], 1, ',', '.') }}%</span>
                        </div>
                        <div class="progress" style="height: 10px;">
                            <div class="progress-bar {{ $projection['is_held_by_speedbump'] ? 'bg-warning' : ($projection['is_ready_mathematically'] ? 'bg-success' : 'bg-primary') }}" 
                                 role="progressbar" 
                                 style="width: {{ min($projection['progress_percentage'], 100) }}%" 
                                 aria-valuenow="{{ min($projection['progress_percentage'], 100) }}" aria-valuemin="0" aria-valuemax="100"></div>
                        </div>
                    </div>
                    ...
                    <div class="d-flex align-items-center bg-light rounded p-3">
                        ...
                        <div>
                            <h5 class="mb-0 text-dark">Estimasi Kenaikan: Tahun {{ $projection['projected_year'] }}</h5>
                            <small class="text-muted">Dibutuhkan sekitar {{ $projection['estimated_years'] }} tahun dari sekarang.</small>
                        </div>
                    </div>
                </div>
            </div>
                    ...
                    <div class="table-responsive">
                        <table class="table table-hover align-middle mb-0">
                            ...
                            <tbody>
                                ...
                                @empty
                                    <tr>
                                        <td colspan="4" class="text-center py-4 text-muted">Belum ada riwayat PAK.</td>
                                    </tr>
                                @endforelse
                            </tbody>
                        </table>
    ...
    document.addEventListener("DOMContentLoaded", function() {
        if (typeof c3 !== 'undefined') {
            ...
            if (chartYears.length > 0 && chartAk.length > 0) {
                ...
                document.getElementById('ak-trend-chart').innerHTML = '<div class="d-flex justify-content-center align-items-center h-100 text-muted">Data trend tidak tersedia.</div>';
            }
